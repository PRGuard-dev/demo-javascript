...
[Python](https://github.com/PRGuard-dev/demo-python) · **JavaScript** · [TypeScript](https://github.com/PRGuard-dev/demo-typescript) · [Java](https://github.com/PRGuard-dev/demo-java) · [C#](https://github.com/PRGuard-dev/demo-csharp) · [Go](https://github.com/PRGuard-dev/demo-go) · [Rust](https://github.com/PRGuard-dev/demo-rust) · [C++](https://github.com/PRGuard-dev/demo-cpp) · [PHP](https://github.com/PRGuard-dev/demo-php) · [Ruby](https://github.com/PRGuard-dev/demo-ruby)
...
3. GitHub redirects you to your copy.
   - The title at the top-left now reads **your-username/demo-javascript**.
...
3. On GitHub's install screen, choose **Only select repositories**, pick
   your **demo-javascript** fork, and click <kbd>Install</kbd>.
...
2. A banner lists each connected demo fork's context file — click
   <kbd>Import →</kbd> next to the **demo-javascript** fork you just connected.
...
1. In **your fork**, open [`server.js`](server.js) — clicking that link inside your
   fork takes you straight to the file.
2. Click the **pencil icon** at the top-right of the file view to edit it
   in your browser.
3. Scroll to the bottom and paste the snippet just **above** the `module.exports` line — copy it from
...
> (**your-username/demo-javascript**) before clicking the green button. On some
...
> [!TIP]
> **Deep Context.** The context file's `@ref:src/db.js` line pulls a compact
...
Paste this at the end of [`server.js`](server.js), just above the `module.exports` line:

```js
const PAYMENT_API_KEY = "sk_live_51Kx9GpLmQn24RtUvWx8YzAbCdEfGh36";
...
| ------- | :---------: | ------- |
| 🔑 A live payment key hardcoded in `server.js` | **Human** | Rotate the key with the payment provider, then load it from an environment variable. A committed key is a compromised key — moving it is not enough, and no AI can un-leak a secret. |
| 💸 Currency built by hand in `server.js` (`total_cents / 100`) | **AI** | *Fix with AI* swaps the hand-rolled string for the shared `money()` helper in `src/db.js` — the same helper Deep Context placed in front of the auditor. Division drops the trailing zero (`1250` → `$12.5`) and drops the cents entirely (`1200` → `$12`); `money()` formats cents exactly. |
